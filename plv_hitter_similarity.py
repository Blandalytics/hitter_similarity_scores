...
def generate_comp_card(player_stats, sim_stats, top_comps,top=True):
    ...
    # gradient_bar(axs[0], [x for x in range(6)]*5, np.where(chart_df['label_text']==f'{player_name} {sim_season}',chart_df['value'],0))
    ...
    axs[0].set_xticklabels(['Aggression+','In-Zone\nDecisions+','Out-of-Zone\nDecisions+','Contact+','Gap Power+','HR Power+'],fontsize=9,color=pl_white)
    axs[0].tick_params(
