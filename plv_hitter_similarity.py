...
def generate_comp_card(player_stats, sim_stats, top_comps,top=True):
    ...
    fig.text(0.16,1.025,f"Hitter Skill\nSimilarity Scores{least_text}",va='center',ha='left',color=pl_text,fontsize=16)
    fig.text(0.345,1.025,f"{top_comps.iloc[0]['Name']} ({sim_season})",va='center',ha='left',color=pl_highglight,fontsize=24)
    # Add PL logo
